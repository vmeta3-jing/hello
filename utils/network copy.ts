...
export interface ChainlinkConfig {
  ...
  subscribeId?: string;
  keyHash?: number | string;
  requestConfirmations?: number | string;
}

export function chlink_conf(networkName: string): ChainlinkConfig {
  const Chain: ChainlinkConfig = {};
  if (networkName) {
    Chain.contract = process.env['CHAINLINK_CONTRACT_' + networkName.toUpperCase()];
    Chain.gasLimit = process.env['CHAINLINK_GAS_LIMIT_' + networkName.toUpperCase()];
    Chain.subscribeId = process.env['CHAINLINK_SUBSCRIBE_ID_' + networkName.toUpperCase()];
    Chain.keyHash = process.env['CHAINLINK_KEY_HASH_' + networkName.toUpperCase()];
    Chain.requestConfirmations = process.env['CHAINLINK_REQUEST_CONFIRMATIONS_' + networkName.toUpperCase()];
  } else {
    Chain.contract = process.env['CHAINLINK_CONTRACT'];
    Chain.gasLimit = process.env['CHAINLINK_GAS_LIMIT'];
    Chain.subscribeId = process.env['CHAINLINK_SUBSCRIBE_ID'];
    Chain.keyHash = process.env['CHAINLINK_KEY_HASH'];
    Chain.requestConfirmations = process.env['CHAINLINK_REQUEST_CONFIRMATIONS'];
  }
